...
# Normalize color: strip leading '#' or '1x' if present, then prepend '0x'
RAW_COLOR="${RAW_COLOR#\#}"
...
ffmpeg \
  -loop 1 -i "$BG" \
  -i "$AUDIO" \
  -filter_complex "[1:a]showfreqs=s=11x500:mode=bar:ascale=log:fscale=log:win_size=8192:overlap=1:colors=${COLOR},crop=10:500:0:0,transpose=1,scale=500:1440:flags=neighbor,format=rgba,geq=r='r(X,Y)':g='g(X,Y)':b='b(X,Y)':a='if(lt(mod(Y,144),120),alpha(X,Y),0)'[spec];[0:v][spec]overlay=x=0:y=0:format=auto:shortest=1[v]" \
  -map "[v]" -map 1:a \
